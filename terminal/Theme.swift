...
enum Theme {
    ...
    /// Terminal's built-in themes: the GitHub Default palettes under terminal's own
    ...
    nonisolated static let defaultDarkDefinition = terminalDefault(
        named: defaultDarkThemeName, from: "GitHub Dark Default", dark: true
    )
    nonisolated static let defaultLightDefinition = terminalDefault(
    ...
    /// terminal backends. Each appearance has 29 catalog themes plus Terminal's
    ...
    /// Terminal's Default comes first; the duplicate GitHub Default catalog rows
    ...
    /// A terminal built-in or catalog theme by name.
    ...
    /// bundled `terminal +themes` browser uses this while the user moves through
    ...
    /// Whether the selected theme for one appearance is a terminal built-in
    /// Default theme, which keeps the sidebar's translucent material.
    nonisolated static func isDefault(dark: Bool) -> Bool {
        terminal(dark: dark).isTerminalDefault
    }

    /// A copy of a catalog theme under a terminal-owned name.
    private nonisolated static func terminalDefault(
...
nonisolated extension GhosttyThemeDefinition {
    /// Whether this is one of terminal's built-in Default themes, which keep the
    /// pre-theme chrome (material sidebar, label-based hairlines).
    var isTerminalDefault: Bool {
    ...
    /// Sidebar fill: terminal's built-in Default themes keep the GitHub
