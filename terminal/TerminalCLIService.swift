...
final class TerminalCLIService {
    ...
    private init() {
        ...
        directoryURL = fileManager.temporaryDirectory
            .appendingPathComponent(
                "terminal-cli-\(ProcessInfo.processInfo.processIdentifier)-\(UUID().uuidString)",
                isDirectory: true
            )
    ...
    private func appearance(from request: TerminalCLIRequest) -> Bool? {
    ...
    private func beginPreview(id: String, pid: pid_t, name: String, dark: Bool) {
        ...
        // An explicit light/dark browse must be visible even when Terminal is
    ...
    private func writeState() {
        ...
        do {
            ...
            NSLog("terminal: failed to write CLI theme catalog: \(error)")
        }
    }
