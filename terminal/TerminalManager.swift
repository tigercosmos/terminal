...
final class TerminalManager: nonisolated ObservableObject {
    ...
    /// Finder service requests use it to target the active Terminal window.
    ...
    /// Folder requests can arrive while macOS is still launching Terminal, before
    ...
    /// reopen Terminal after the user has closed its last window.
    ...
    /// launched there. Used by Terminal's Finder service.
    ...
    /// Commands are constructed during app launch even when macOS starts Terminal
    ...
    /// Opens a browser tab in the current project. Terminal remains
