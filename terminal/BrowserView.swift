...
private enum BrowserContextMenuSupport {
    static let messageName = "terminalContextMenuLink"
    static let script = #"""
    (() => {
      window.addEventListener("contextmenu", event => {
        ...
        window.webkit.messageHandlers.terminalContextMenuLink.postMessage(
          link?.href || ""
        );
      }, true);
    })()
...
final class BrowserWebView: WKWebView {
    ...
    private static let openLinkInNewTabIdentifier =
        NSUserInterfaceItemIdentifier("terminal.browser.openLinkInNewTab")
    private static let openLinkInNewPaneIdentifier =
        NSUserInterfaceItemIdentifier("terminal.browser.openLinkInNewPane")
...
final class BrowserTab: NSObject, ObservableObject, Identifiable, WKNavigationDelegate, WKUIDelegate {
    ...
    /// Keep target=_blank links inside this browser tab. Terminal has an explicit
...
/// so it follows Terminal's appearance while the page remains a real WKWebView.
