...
final class FileTab: nonisolated ObservableObject, nonisolated Identifiable {
    ...
    ///
    /// Terminal's premise is that an agent is working in the same tree, so "the
...
struct FileViewerView: View {
    ...
    var body: some View {
        ...
        // The selected file view stays mounted while Terminal is inactive, so
        ...
        // inactive (for example when moving between Terminal windows).
        .onReceive(NotificationCenter.default.publisher(
