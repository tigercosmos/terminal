...
/// here is Terminal's: cell layout, glyph drawing, the cursor, selection, and the
...
final class AlacrittyTerminalView: NSView, TerminalBackendSurface, NSUserInterfaceValidations {
    ...
    /// Matches the window padding Terminal's Ghostty panes use, so a pane looks
    ...
    /// Terminal is inactive. Retaining one frame is much cheaper than keeping the
    ...
    deinit {
        ...
        if let handle { terminal_alacritty_free(handle) }
    }
    ...
    private func start(launch: TerminalLaunch) {
        ...
        if handle == nil {
            NSLog("terminal: failed to start the Alacritty backend for \(launch.program)")
            return
        }
    }
    ...
    /// Terminal windows, but release the CAMetalLayer drawable pool and renderer.
    ...
    /// Re-renders the retained image while Terminal or its terminal window is not
    ...
    private func reportWorkingDirectory() {
        ...
        let pid = terminal_alacritty_foreground_pid(handle)
        guard let path = processWorkingDirectory(pid: pid)
            ?? processWorkingDirectory(pid: terminal_alacritty_child_pid(handle))
        else { return }
    ...
    func detach() {
        ...
        guard let handle else { return }
        self.handle = nil
        terminal_alacritty_free(handle)
    }
    ...
    private func renderFrame(
        ...
        guard let renderer = metalRenderer else {
            NSLog("terminal: no Metal device; the Alacritty backend cannot draw")
            return false
        }
        ...
        terminal_alacritty_take_damage(handle, &damage)
        ...
        var dirtyRows: [Int]?
        if needsUnconditionalRedraw || damage.kind == TERMINAL_DAMAGE_FULL {
            dirtyRows = nil
    ...
    private func applyHoveredURLUnderline(to snapshot: inout TerminalSnapshot) {
    ...
    private func updateKittyGraphics(handle: OpaquePointer) {
        var snapshot = TerminalKittySnapshot()
        terminal_alacritty_kitty_snapshot(handle, &snapshot)
    ...
    private func updateMarkedTextOverlay(snapshot: TerminalSnapshot) {
    ...
    /// Feeds Terminal's overlay scrollbar.
    ///
    ...
    private func reportScroll() {
        ...
        // scrollback; Terminal's scrollbar measures down from the oldest row.
    ...
    func handleEvent(kind: UInt32, payload: Data) {
        if events == nil,
           kind == TERMINAL_EVENT_TITLE
            || kind == TERMINAL_EVENT_BELL
            || kind == TERMINAL_EVENT_EXIT
            || kind == TERMINAL_EVENT_CLIPBOARD_LOAD
            || kind == TERMINAL_EVENT_WORKING_DIRECTORY
            || kind == TERMINAL_EVENT_NOTIFICATION
            || kind == TERMINAL_EVENT_SHELL_PROMPT_START
            || kind == TERMINAL_EVENT_SHELL_COMMAND_START
            || kind == TERMINAL_EVENT_SHELL_COMMAND_EXECUTING
            || kind == TERMINAL_EVENT_SHELL_COMMAND_FINISHED {
        ...
        case TERMINAL_EVENT_CLIPBOARD_LOAD:
        ...
        case TERMINAL_EVENT_WORKING_DIRECTORY:
    ...
    private func resolveClipboard(
        ...
        bytes.withUnsafeBufferPointer { pointer in
            terminal_alacritty_resolve_clipboard(
    ...
    var hasSelection: Bool {
        ...
        return terminal_alacritty_has_selection(handle)
    }
    ...
    func findSelection() {
        guard let handle, terminal_alacritty_has_selection(handle) else { return }
        let needed = terminal_alacritty_selection_text(handle, nil, 0)
        guard needed > 0 else { return }
        var buffer = [UInt8](repeating: 0, count: needed)
        let written = buffer.withUnsafeMutableBufferPointer { pointer in
            terminal_alacritty_selection_text(handle, pointer.baseAddress, needed)
        }
    ...
    private func exportFile(scrollbackOnly: Bool) -> String? {
        guard let handle else { return nil }
        let needed = terminal_alacritty_buffer_text(handle, scrollbackOnly, nil, 0)
        guard needed > 0 else { return nil }

        var buffer = [UInt8](repeating: 0, count: needed)
        let written = buffer.withUnsafeMutableBufferPointer { pointer in
            terminal_alacritty_buffer_text(handle, scrollbackOnly, pointer.baseAddress, needed)
        }
        guard written > 0 else { return nil }

        let manager = FileManager.default
        let directory = manager.temporaryDirectory
            .appendingPathComponent("terminal-export-\(UUID().uuidString)", isDirectory: true)
        do {
    ...
    private func write(_ bytes: [UInt8]) {
        ...
        bytes.withUnsafeBufferPointer { pointer in
            terminal_alacritty_write(handle, pointer.baseAddress, pointer.count)
        }
        resetCursorBlink()
    }
    ...
    private func writeControl(_ bytes: [UInt8]) {
        guard let handle, !bytes.isEmpty else { return }
        bytes.withUnsafeBufferPointer { pointer in
            terminal_alacritty_write_control(handle, pointer.baseAddress, pointer.count)
        }
    }

    private var terminalMode: AlacrittyTerminalMode {
        guard let handle else { return [] }
        return AlacrittyTerminalMode(rawValue: terminal_alacritty_mode(handle))
    }
    ...
    override func mouseDragged(with event: NSEvent) {
        ...
        let point = gridPoint(for: event)
        terminal_alacritty_selection_update(
    ...
    private func url(
        ...
        guard let handle else { return nil }
        let needed = terminal_alacritty_url_at(
            handle, Int32(point.line), point.column, nil, nil, 0
        )
        guard needed > 0 else { return nil }
        var buffer = [UInt8](repeating: 0, count: needed)
        var range = TerminalURLRange()
        let written = buffer.withUnsafeMutableBufferPointer { pointer in
            terminal_alacritty_url_at(
    ...
    override func selectAll(_ sender: Any?) {
        guard let handle else { return }
        terminal_alacritty_select_all(handle)
        scheduleRender(force: true)
    }
    ...
    /// Terminal reserves right-click for its terminal/pane menu, matching the
...
extension TerminalLaunch {
    /// Builds a `TerminalConfig` whose C strings stay alive for the call. Cargo's
...
enum AlacrittyTheme {
    /// Terminal's active terminal theme in the flat form the bridge resolves
    /// colors against, so Alacritty panes match Ghostty panes exactly.
    static func current() -> AlacrittyPalette {
        let isDark = NSApp.effectiveAppearance.bestMatch(from: [.darkAqua, .aqua]) == .darkAqua
        let definition = Theme.terminal(dark: isDark)

        var theme = AlacrittyPalette()
