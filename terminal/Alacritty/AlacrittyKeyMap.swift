...
/// `TERMINAL_MODE_*` bits the Rust bridge reports.
...
/// whatever owns the keyboard, which here is Terminal. This covers the xterm
...
enum AlacrittyKeyMap {
    ...
    /// (IME composition, or a shortcut Terminal's menus own).
    static func bytes(
        for event: NSEvent,
        ...
        if flags.contains(.command) {
            ...
            // Command shortcuts still fall through to Terminal's menus.
