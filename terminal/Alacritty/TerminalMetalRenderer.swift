...
/// Terminal's editor and to the Ghostty backend, which is the whole reason the
...
final class TerminalMetalRenderer {
    ...
    init?(device: MTLDevice) {
        ...
        guard let library = try? device.makeLibrary(source: Self.shaderSource, options: nil),
              let vertexFunction = library.makeFunction(name: "terminal_terminal_vertex"),
              let fragmentFunction = library.makeFunction(name: "terminal_terminal_fragment")
        else { return nil }
    ...
    private func buildRow(
        ...
        for column in 0..<columns {
            let cell = cells[row * columns + column]
            guard cell.flags & UInt16(TERMINAL_CELL_HIDDEN) == 0,
                  cell.flags & UInt16(TERMINAL_CELL_WIDE_SPACER) == 0
            else { continue }
            ...
            if cell.ch != 0,
               cell.text_len > 0 || cell.ch != UInt32(UInt8(ascii: " ")) {
                ...
                let key = TerminalGlyphAtlas.Key(
                    content: content,
                    bold: cell.flags & UInt16(TERMINAL_CELL_BOLD) != 0,
                    italic: cell.flags & UInt16(TERMINAL_CELL_ITALIC) != 0
                )
    ...
    private func blockCursorInsertionIndex(snapshot: TerminalSnapshot) -> Int? {
    ...
    fragment float4 terminal_terminal_fragment(
