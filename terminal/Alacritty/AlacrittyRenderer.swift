...
struct AlacrittyRenderer {
    ...
    private func drawGlyphs(
        ...
        for column in 0..<columns {
            let cell = cells[row * columns + column]
            guard cell.flags & UInt16(TERMINAL_CELL_HIDDEN) == 0 else { continue }
            // A wide char's trailing spacer carries no glyph of its own.
            guard cell.flags & UInt16(TERMINAL_CELL_WIDE_SPACER) == 0 else { continue }
            ...
            let isBold = cell.flags & UInt16(TERMINAL_CELL_BOLD) != 0
            let isItalic = cell.flags & UInt16(TERMINAL_CELL_ITALIC) != 0
            ...
            if cell.flags & UInt16(TERMINAL_CELL_UNDERLINE) != 0 {
                line(at: y - metrics.cellHeight * 0.12, x: x, color: foreground, in: context)
            }
            if cell.flags & UInt16(TERMINAL_CELL_STRIKEOUT) != 0 {
    ...
    private func drawCursor(snapshot: TerminalSnapshot, in context: CGContext, bounds: CGRect) {
    ...
    /// how to present them.
    static func foreground(of cell: TerminalCell, default background: UInt32) -> UInt32 {
        var color = cell.flags & UInt16(TERMINAL_CELL_INVERSE) != 0 ? cell.bg : cell.fg
        if cell.flags & UInt16(TERMINAL_CELL_DIM) != 0 {
            color = dim(color)
        }
        if cell.flags & UInt16(TERMINAL_CELL_SELECTED) != 0 {
    ...
    static func background(of cell: TerminalCell, default background: UInt32) -> UInt32 {
        if cell.flags & UInt16(TERMINAL_CELL_SELECTED) != 0 {
            return cell.flags & UInt16(TERMINAL_CELL_INVERSE) != 0 ? cell.bg : cell.fg
        }
        return cell.flags & UInt16(TERMINAL_CELL_INVERSE) != 0 ? cell.fg : cell.bg
    }
