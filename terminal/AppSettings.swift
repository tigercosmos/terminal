...
final class AppSettings: nonisolated ObservableObject {
    ...
    /// Development (Debug) builds store their config under `~/.config/terminal-dev`
    /// instead of `~/.config/terminal`, so running a dev build alongside an
    /// installed production build doesn't clobber its settings. This mirrors
    /// the separate `com.github.tigercosmos.terminal.dev` bundle identifier that keeps the two apps'
    ...
    /// definitions (terminal built-ins plus the ghostty catalog).
    ...
    /// Persisted as `terminal.font-thicken`; off by default so Terminal's text
    ...
    private func save() {
        ...
        do {
            ...
            NSLog("terminal: failed to write \(Self.configURL.path): \(error)")
        }
    }
