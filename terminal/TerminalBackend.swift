...
///
/// Both backends implement Terminal's own surface vocabulary, keeping emulator
...
enum TerminalBackend: String, CaseIterable, Identifiable, Sendable {
    ...
    /// Terminal's real surface identity, distinct from `TERM_PROGRAM`, which is
    /// a compatibility hint for terminal protocols.
    ...
    /// selection that cannot take effect would misreport what Terminal will do.
    ...
    /// Ghostty protocols Terminal implements, including OSC 777 notifications
    /// and OSC 9;4 progress reports.
...
/// Everything a backend needs to start one pane's shell. Terminal resolves the
...
/// What Terminal needs from a terminal surface, in Terminal's vocabulary rather than
/// any one emulator's.
///
...
protocol TerminalBackendSurface: NSView, TerminalFindSurface {
    ...
    /// True only while Terminal is active, its window is key, and this exact
    ...
    ///
    /// Named for Terminal's UI (⌘F, `TerminalFind`) rather than for any backend's
    ...
    /// needle backend-side so Terminal never has to read it out and re-escape it.
    func findSelection()
    ...
    /// temporary directory: Terminal validates that before reading, and deletes
    ...
    /// viewport alone. Nil when there is none — which is also how Terminal tells a
...
protocol TerminalBackendEvents: AnyObject {
    ...
    /// The backend started a find of its own accord — Terminal's ⌘E path resolves
...
/// A backend asking Terminal to confirm clipboard access before it proceeds.
