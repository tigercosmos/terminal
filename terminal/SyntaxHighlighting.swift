...
// The one grammar terminal vendors itself, for `SyntaxLanguage.tsx`.
import TreeSitterTSX

/// A grammar terminal can highlight with: one of the plugin's bundled languages,
/// or TSX, which terminal vendors.
///
...
enum SyntaxHighlighting {
    /// The theme terminal hands the highlighter: the plugin's bundled *default
    ...
    /// discarding terminal's `TerminalFont.current()`. With no fonts, every token
    /// keeps the editor's own font and only the foreground color changes.
    ///
    /// `STPluginNeonAppKit.Theme` is fully qualified because terminal has its own
