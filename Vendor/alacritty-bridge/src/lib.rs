...
pub const TERMINAL_EVENT_MOUSE_SHAPE: u32 = 13;
...
/// Termy solves this at the PTY boundary; Terminal uses the same seam so the
...
impl OscInterceptor {
    ...
    fn parse_payload(&self) -> Option<OscEvent> {
        ...
        if let Some(name) = payload.strip_prefix("22;") {
            // OSC 22 pointer shape. Like Ghostty, Terminal takes a single CSS
...
/// buffers the enclosed bytes atomically, but Terminal's host-driven cursor
/// timer can otherwise request a frame while that buffer is still being
/// assembled. Pointer shape: Ghostty's stream handler moves the pointer when
/// mouse reporting toggles and when RIS lands, and the emulator reports
/// neither.
#[derive(Debug, Default)]
...
/// `OSC 777 ; notify ; title ; body ST`. Terminal presents its own app title,
/// so prefer the body and fall back to the protocol title when no body is sent.
...
impl Proxy {
    ...
    fn emit_osc(&self, event: OscEvent) {
        match event {
            ...
            OscEvent::MouseShape(name) => self.emit(TERMINAL_EVENT_MOUSE_SHAPE, name.as_bytes()),
        }
    }
}
